...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        #max_state = max(self.Q[state])          # Max of the current state
        #maxQ = self.Q[state][max_state]
        #max_state = max(self.Q[state], key = lambda x: self.Q[state][x])
        #maxQ = self.Q[state][max_state]
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            if random.random() > self.epsilon:
                ...
                action = random.choice([action for action in self.valid_actions if self.Q[state][action] == maxV])
            else:
                action = random.choice(self.valid_actions)
        else:
            action = random.choice(self.valid_actions)
